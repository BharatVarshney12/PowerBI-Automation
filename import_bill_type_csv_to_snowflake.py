"""
Import CSV Data to Snowflake - Spend by Bill Type
Imports Spend by bill type.csv into PowerBI_learning.TRAINING_POWERBI.SPEND_BY_BILL_TYPE table
"""
...
def import_bill_type_csv_to_snowflake():
    """Import Spend by Bill Type CSV data to Snowflake table and verify"""
    
    test_name = "Import Bill Type CSV to Snowflake"
    log_test_start(test_name)
    
    print("\n" + "="*80)
    print("IMPORTING SPEND BY BILL TYPE CSV TO SNOWFLAKE")
    print("="*80)
    
    try:
        # Step 1: Load CSV file
        log_step("Load CSV File")
        csv_file = Path(__file__).parent / 'Spend by  bill type.csv'
        ...
        column_mapping = {
            ...
            'Bill Type Code': 'BILL_TYPE',  # Column in Snowflake is BILL_TYPE, not BILL_TYPE_CODE
        ...
        # Convert BILL_TYPE to string (it can be numeric like "131" or text like "Total")
        df['BILL_TYPE'] = df['BILL_TYPE'].astype(str)
        
        # Replace NaN with None (NULL in Snowflake) for all columns
        ...
        with SnowflakeConnector() as sf_conn:
            ...
            log_step("Check if table SPEND_BY_BILL_TYPE exists")
            
            check_table_query = """
            SELECT COUNT(*) as TABLE_EXISTS
            FROM INFORMATION_SCHEMA.TABLES 
            WHERE TABLE_SCHEMA = 'TRAINING_POWERBI' 
            AND TABLE_NAME = 'SPEND_BY_BILL_TYPE'
            """
            ...
            if not table_exists:
                log_error("Table SPEND_BY_BILL_TYPE does not exist")
                raise Exception("Table SPEND_BY_BILL_TYPE does not exist in PowerBI_learning.TRAINING_POWERBI schema")
            
            print(f"[SNOWFLAKE]  Table SPEND_BY_BILL_TYPE exists")
            
            # Step 4: Get table structure
            log_step("Get table structure")
            
            describe_query = "DESCRIBE TABLE SPEND_BY_BILL_TYPE"
            ...
            log_action("Truncating table SPEND_BY_BILL_TYPE")
            sf_conn.cursor.execute("TRUNCATE TABLE SPEND_BY_BILL_TYPE")
            ...
            insert_query = f"INSERT INTO SPEND_BY_BILL_TYPE ({columns}) VALUES ({placeholders})"
            ...
            for index, row in df.iterrows():
                try:
                    values = tuple(row)
                    if index < 5:  # Show first 5 rows for debugging
                        print(f"[DEBUG] Row {index}: {values}")
                    sf_conn.cursor.execute(insert_query, values)
                    inserted_count += 1
                    if index < 5 or (index + 1) % 10 == 0:  # Show progress every 10 rows
            ...
            verify_query = "SELECT COUNT(*) as ROW_COUNT FROM SPEND_BY_BILL_TYPE"
            ...
            sample_query = "SELECT * FROM SPEND_BY_BILL_TYPE LIMIT 10"
            ...
            full_query = "SELECT * FROM SPEND_BY_BILL_TYPE ORDER BY CLAIM_FORM_TYPE, BILL_TYPE"
            snowflake_full_data = sf_conn.execute_query(full_query)
            
            # Sort CSV data the same way
            df_sorted = df.sort_values(by=['CLAIM_FORM_TYPE', 'BILL_TYPE']).reset_index(drop=True)
            ...
            print(f" Snowflake table exists: SPEND_BY_BILL_TYPE")
...
if __name__ == "__main__":
    import_bill_type_csv_to_snowflake()

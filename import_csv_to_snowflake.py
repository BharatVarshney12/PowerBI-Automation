"""
Import CSV Data to Snowflake and Verify
Imports spend by code.csv into POWERBI_LEARNING.SPEND_BY_CODE table
"""
...
def import_csv_to_snowflake():
    """Import CSV data to Snowflake table and verify"""
    
    test_name = "Import CSV to Snowflake"
    log_test_start(test_name)
    
    print("\n" + "="*80)
    print("IMPORTING CSV DATA TO SNOWFLAKE")
    print("="*80)
    
    try:
        # Step 1: Load CSV file
        log_step("Load CSV File")
        csv_file = Path(__file__).parent / 'spend by code.csv'
        ...
        with SnowflakeConnector() as sf_conn:
            ...
            log_step("Check if table SPEND_BY_CODE exists")
            ...
            if not table_exists:
                log_error("Table SPEND_BY_CODE does not exist")
                raise Exception("Table SPEND_BY_CODE does not exist in PowerBI_learning.TRAINING_POWERBI schema")
            
            print(f"[SNOWFLAKE]  Table SPEND_BY_CODE exists")
            
            # Step 4: Get table structure
            log_step("Get table structure")
            
            describe_query = "DESCRIBE TABLE SPEND_BY_CODE"
            ...
            log_action("Truncating table SPEND_BY_CODE")
            sf_conn.cursor.execute("TRUNCATE TABLE SPEND_BY_CODE")
            ...
            insert_query = f"INSERT INTO SPEND_BY_CODE ({columns}) VALUES ({placeholders})"
            ...
            verify_query = "SELECT COUNT(*) as ROW_COUNT FROM SPEND_BY_CODE"
            ...
            sample_query = "SELECT * FROM SPEND_BY_CODE LIMIT 5"
            sample_data = sf_conn.execute_query(sample_query)
            
            print(f"\n[SNOWFLAKE] Sample Data (first 5 rows):")
            print(sample_data)
            ...
            full_query = "SELECT * FROM SPEND_BY_CODE ORDER BY CLAIM_FORM_TYPE"
            snowflake_full_data = sf_conn.execute_query(full_query)
            
            # Sort CSV data the same way
            df_sorted = df.sort_values(by='CLAIM_FORM_TYPE').reset_index(drop=True)
            ...
            print(f" Snowflake table exists: SPEND_BY_CODE")

...
def generate_validation_report(table_config):
    ...
    if not missing_in_sf and not extra_in_sf:
        ...
        if extra_in_sf:
            print(f"   ⚠️  Extra columns in Snowflake: {extra_in_sf}")
    ...
    if dtype_results['mismatches']:
        print(f"   ⚠️  Found {len(dtype_results['mismatches'])} data type issues:")
    ...
    if null_results['differences']:
        print(f"   ⚠️  Found null count differences in {len(null_results['differences'])} columns:")
    ...
    if sample_results['mismatches']:
        print(f"   ⚠️  Found {len(sample_results['mismatches'])} data mismatches in sample:")
    ...
    if all_passed:
        ...
        print("⚠️  OVERALL STATUS: ❌ SOME VALIDATIONS FAILED - Review details above")
        ...
        for report in reports:
            ...
            summary_data = {
                ...
                'Status': [
                    ...
                    '✅ PASS' if not report['validations']['data_types']['mismatches'] else '⚠️ WARN',
                    '✅ PASS' if not report['validations']['null_counts']['differences'] else '⚠️ WARN',
